...
from simulation import TIME_UNIT_SECONDS, calculate_qber

logging.basicConfig(level=logging.INFO,
                    format="%(asctime)s - %(levelname)s - %(message)s")


def run_comparison(channel_length_km,
    ...
    for run in range(num_runs):
        logging.info(
            f"Run {run+1}/{num_runs} for distance {channel_length_km}km")
        network, nodes = create_qkd_network(2, channel_length_km,
        ...
        rx_ids = []
        protocols = [
            TransmitterProtocol(nodes[0],
                                final_key,
                                sent_bits,
                                1,
                                sent_bases=sent_bases,
                                pulse_ids=tx_ids),
            ReceiverProtocol(nodes[1],
                             final_key,
                             sent_bits,
                             measured_bits,
                             0,
    ...
    for source_type in comparison_results:
        ...
        if rates:
            ...
            comparison_results[source_type]['rate_std'] = np.std(rates) if len(
                rates) > 1 else 0
            comparison_results[source_type]['qber'] = sum(qbers) / len(qbers)
            comparison_results[source_type]['qber_std'] = np.std(qbers) if len(
                qbers) > 1 else 0
        logging.info(
            f"{source_type}: rate={comparison_results[source_type]['rate']:.4f}±{comparison_results[source_type].get('rate_std', 0):.4f}, QBER={comparison_results[source_type]['qber']:.4f}±{comparison_results[source_type].get('qber_std', 0):.4f}"
        )

    return comparison_results


def run_distance_sweep(output_dir,
                       detector_efficiency=1.0,
                       loss_per_km=0.2,
                       simulation_duration=300,
                       num_runs=3):
    ...
    for dist in distances:
        ...
        results[dist] = run_comparison(dist, detector_efficiency, loss_per_km,
...
if __name__ == "__main__":
    ...
    results = run_distance_sweep(COMPARE_OUTPUT_DIR)
