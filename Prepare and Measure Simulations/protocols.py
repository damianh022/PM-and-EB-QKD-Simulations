...
from netsquid.qubits.ketstates import h0, h1, s0, s1
...
class TransmitterProtocol(NodeProtocol):
    ...
    def run(self):
        while not self.completed:
            ...
            logging.debug(
                f"Transmitter {self.node.name} generated bit: {bit} in basis: {basis}, pid={pulse_id}"
            )
            ...
            logging.debug(
                f"Qubit sent from {self.node.name} to Node_{self.target_node_index} (pid={pulse_id})"
            )
            ...
            logging.debug(
                f"Basis sent from {self.node.name} to Node_{self.target_node_index} (pid={pulse_id})"
            )
            ...
            if self.target_length and len(self.final_key) >= self.target_length:
                logging.info(
                    f"Transmitter {self.node.name} reached target key length of {self.target_length}"
                )
                self.completed = True
                break
...
class ReceiverProtocol(NodeProtocol):
    ...
    def run(self):
        while not self.completed:
            logging.debug(
                f"Waiting for qubit at {self.node.name} qin_{self.source_node_index}"
            )
            yield self.await_port_input(
                self.node.ports[f'qin_{self.source_node_index}'])
            port_input = self.node.ports[
                f'qin_{self.source_node_index}'].rx_input()
            if port_input is None or len(port_input.items) == 0:
                logging.debug(
                    f"No qubit received at qin_{self.source_node_index}")
                # Still wait for/classically receive basis to avoid backlog
                yield self.await_port_input(
                    self.node.ports[f'cin_{self.source_node_index}'])
                _ = self.node.ports[f'cin_{self.source_node_index}'].rx_input()
                continue
            qubit = port_input.items[0]
            logging.debug(
                f"Qubit received at {self.node.name} qin_{self.source_node_index}"
            )

            # Simple detection efficiency model
            if random.random() > self.detection_efficiency:
                logging.debug(
                    f"Detection failed due to inefficiency at {self.node.name}")
            ...
            logging.debug(
                f"Waiting for message at {self.node.name} cin_{self.source_node_index}"
            )
            yield self.await_port_input(
                self.node.ports[f'cin_{self.source_node_index}'])
            message = self.node.ports[f'cin_{self.source_node_index}'].rx_input(
            ).items[0]
            sent_basis = message['basis']
            pulse_id = message.get('pulse_id')
            logging.debug(
                f"Received basis over classical channel (pid={pulse_id}, basis={sent_basis})"
            )
            ...
            if basis == sent_basis:
                ...
                logging.debug(
                    f"Bases match. Measurement result: {measurement} added to key"
                )

                # Check if we've reached the target key length
                if self.target_length and len(
                        self.final_key) >= self.target_length:
                    logging.info(
                        f"Receiver {self.node.name} reached target key length of {self.target_length}"
                    )
                    self.completed = True
                    break
            else:
...
class RelayProtocol(NodeProtocol):
    ...
    def __init__(self,
        ...
        source_handler = SourceLinkHandler(self.node, self.source_key,
                                           self.source_node_index,
                                           self.target_length)
        target_handler = TargetLinkHandler(self.node, self.target_key,
    ...
    def _pre_generate_target_key(self):
        ...
        self._pre_generated_target_bits = [
            random.randint(0, 1) for _ in range(self.target_length * 2)
        ]
    ...
    def run(self):
        ...
        while not self.completed:
            ...
            if len(self.source_key) >= self.target_length and len(
                    self.target_key) >= self.target_length:
                logging.info(
                    f"Relay {self.node.name} has completed both keys: Source: {len(self.source_key)}, Target: {len(self.target_key)}"
                )
                ...
                logging.info(
                    f"Relay {self.node.name} completed XORed key of length {len(self.final_key)}"
                )
                self.completed = True
                break
            ...
            if ns.sim_time() % 20 == 0:  # Every 20 time units
                logging.debug(
                    f"Relay {self.node.name} progress: Source key: {len(self.source_key)}/{self.target_length}, Target key: {len(self.target_key)}/{self.target_length}"
                )
...
class SourceLinkHandler(NodeProtocol):
    ...
    def run(self):
        logging.info(
            f"Starting source link handler for {self.node.name} with source {self.source_node_index}"
        )
        while len(self.source_key) < self.target_length:
            ...
            if port_input is None or len(port_input.items) == 0:
                logging.warning(
                    f"No qubit received from source at {self.node.name}")
                continue
            ...
            measurement, _ = qapi.measure(qubit, observable=Z)
            logging.debug(
                f"{self.node.name} received and measured qubit from source: {measurement}"
            )
            ...
            if len(self.source_key) % 10 == 0:
                logging.debug(
                    f"{self.node.name} source key progress: {len(self.source_key)}/{self.target_length}"
                )
...
class TargetLinkHandler(NodeProtocol):
    ...
    def run(self):
        logging.info(
            f"Starting target link handler for {self.node.name} with target {self.target_node_index}"
        )
        ...
        while len(self.target_key) < self.target_length:
            ...
            if len(self.target_key) % 10 == 0:
                logging.debug(
                    f"{self.node.name} target key progress: {len(self.target_key)}/{self.target_length}"
                )
