...
from netsquid.nodes import DirectConnection, Network, Node
...
DEFAULT_OUTPUT_DIR = os.path.normpath(
    os.path.join(SCRIPT_DIR, "..", "EB QKD Results"))
...
class EntanglementSourceProtocol(NodeProtocol):
    ...
    def __init__(self,
                 ...
                 downstream_ready_ports=None,
                 **kwargs):
        super().__init__(node)
        self.connections = connections
        self.pair_generation_rate = pair_generation_rate
        self.distributed_pairs = 0
        self.downstream_ready_ports = downstream_ready_ports or {
        }  # {node_id: port_name}
            ...
            for node_id1, node_id2 in self.connections:
                ...
                if self.downstream_ready_ports:
                    if node_id1 in self.downstream_ready_ports:
                        yield self.await_port_input(self.node.ports[
                            self.downstream_ready_ports[node_id1]])
                    if node_id2 in self.downstream_ready_ports:
                        yield self.await_port_input(self.node.ports[
                            self.downstream_ready_ports[node_id2]])
                # Emit entangled pair
                ...
                logging.debug(
                    f"Source distributed pair #{self.distributed_pairs} to nodes {node_id1} and {node_id2}"
                )
...
class NodeMeasurementProtocol(NodeProtocol):
    ...
    def __init__(self,
        ...
        logging.info(
            f"Initialized NodeMeasurementProtocol for {node.name} (swapping={self.enable_swapping}, expected_depth={self.expected_depth})"
        )
    ...
    def run(self):
        ...
        for port_name in self.node.ports:
            if port_name.startswith("qin_Source_"):
                source_name = port_name[4:]
                self.node.ports[port_name].bind_input_handler(
                    create_qubit_handler(source_name))
        ...
        for other_node in self.connected_nodes:
            ...
            if port_name in self.node.ports:
                self.node.ports[port_name].bind_input_handler(
                    create_classical_handler(other_node))
        ...
        for port_name in self.node.ports:
            if port_name.startswith("cin_") and port_name not in already:
                ...
                if suffix.isdigit():
                    self.node.ports[port_name].bind_input_handler(
                        create_classical_handler(int(suffix)))
    ...
    def _handle_qubit_with_source(self, message, source_name):
        try:
            if message is None or not hasattr(message,
                                              'items') or not message.items:
                return
            qubit = message.items[0]
            basis = random.choice(['Z', 'X'])
            meas, _ = qapi.measure(qubit, Z if basis == 'Z' else X)
            index = len(self.measurements.get(source_name, []))
            self.measurements.setdefault(source_name, []).append(
                (basis, meas, index))

            if self.enable_swapping:
                self.pending_measurements[source_name].append(
                    (index, basis, meas))
            ...
            for other_node, src_name in self.node_to_source_map.items():
                if src_name == source_name:
                    ...
                    if port_name in self.node.ports:
                        self.node.ports[port_name].tx_output(
                            (self.node_id, source_name, index, basis, meas))
                        logging.debug(
                            f"{self.node.name} measured {meas} in {basis} for {source_name}"
                        )
                    break
    ...
    def _try_process_waiting_swap(self, source_name, force_token_id=None):
        if not self.pending_measurements.get(
                source_name) or not self.waiting_swaps.get(source_name):
            return
        token = None
        if force_token_id is not None:
            ...
            for i, t in enumerate(self.waiting_swaps[source_name]):
                ...
                if tid in self.partner_pending_tokens or rec.get(
        ...
        for candidate in (f'cout_{partner_id}', f'cout_{swapper_id}'
                          if swapper_id is not None else None):
            if candidate and candidate in self.node.ports:
                out_port = candidate
                break
        if out_port:
            self.node.ports[out_port].tx_output(
                ('PAIR', token_id, self.node_id, source_name, idx, basis, bit))
        else:
            logging.debug(
                f"{self.node.name} could not route PAIR token {token_id}")

        self._finalize_pair_if_ready(token_id)

    def _receive_swap_notification(self,
                                   token_id,
                                   swapper_id,
                                   partner_id,
                                   my_source,
        ...
        if self.expected_depth is not None and depth < self.expected_depth:
            return
        local_src = self.node_to_source_map.get(
            from_id) if from_id is not None else my_source
    ...
    def _finalize_pair_if_ready(self, token_id):
        ...
        if not rec or rec.get('completed'):
            return
        if not (all(k in rec for k in ('my_basis', 'my_bit')) and
                all(k in rec
                    for k in ('partner_basis', 'partner_bit', 'partner_id'))):
            return
        ...
        if rec['partner_basis'] == my_basis:
            ...
            if self.node_id < partner_id:
                ...
                self.original_bits.setdefault(partner_id,
                                              {})[token_id] = partner_bit
                self._key_counter += 1
                if self._key_counter % self.key_log_every == 0:
                    logging.info(
                        f"✅ KEY BIT (swap): {self.node.name} adds bit {my_bit} with Node_{partner_id} (token {token_id}, basis {my_basis})"
                    )
        rec['completed'] = True
        if token_id in self.partner_pending_tokens:
    ...
    def _handle_classical_with_node(self, message, sender_node_id):
        try:
            if message is None:
                return
            payload = message.items[0] if hasattr(
                message, 'items') and message.items else message

            if self.enable_swapping and isinstance(payload,
                                                   (tuple, list)) and payload:
                tag = payload[0]
                if tag == 'SWAP' and len(payload) >= 9:
                    _, token_id, swapper_id, partner_id, my_source, partner_source, parity_z, parity_x, depth = payload[:
                                                                                                                        9]
                    self._receive_swap_notification(token_id,
                                                    swapper_id,
                                                    partner_id,
                                                    my_source,
                                                    partner_source,
                ...
                if tag == 'PAIR' and len(payload) >= 7:
                    ...
                    if 'my_source' not in rec:
                        ...
                        self._try_process_waiting_swap(rec['my_source'],
                                                       force_token_id=token_id)
                    self._finalize_pair_if_ready(token_id)
                    return
            ...
            if isinstance(payload, (tuple, list)) and len(payload) >= 5:
                ...
                if source_name in self.measurements:
                    measured_indices = [
                        m[2] for m in self.measurements[source_name]
                    ]
                    if index in measured_indices:
                        idx = measured_indices.index(index)
                        own_basis, own_bit, _ = self.measurements[source_name][
                            idx]
                        if own_basis == basis:
                            key_bit = own_bit
                            self.key_bits.setdefault(sender_id,
                                                     {})[index] = key_bit
                            self.original_bits.setdefault(
                                sender_id, {})[index] = remote_meas
                            key_id = f"{min(self.node_id, sender_id)}-{max(self.node_id, sender_id)}"
                            self.shared_keys.setdefault(key_id,
                                                        []).append(key_bit)
                            logging.debug(
                                f"KEY BIT: {self.node.name} adds bit {key_bit} to key with Node_{sender_id}"
                            )
        except Exception as e:
            logging.warning(
                f"Error handling classical message in {self.node.name}: {e}")
...
class EntanglementSwappingProtocol(NodeProtocol):
    ...
    def __init__(self,
        ...
        if path and self.node_id in path:
            ...
            if idx > 0:
                self.left_neighbor = path[idx - 1]
            if idx < len(path) - 1:
                self.right_neighbor = path[idx + 1]
        self.num_hops = num_hops
        logging.info(
            f"Initialized EntanglementSwappingProtocol at {node.name} (left={self.left_neighbor}, right={self.right_neighbor}, hops={self.num_hops})"
        )
    ...
    def _handle_incoming_qubit(self, message, source_name):
        try:
            ...
            logging.debug(
                f"{self.node.name} stored qubit from Node_{neighbor_id} via {source_name}"
            )
        except Exception as e:
            logging.warning(
                f"Error handling incoming qubit in {self.node.name}: {e}")
    ...
    def _perform_swap(self, node1, node2):
        ...
        if not self.stored_qubits.get(node1) or not self.stored_qubits.get(
                node2):
            return
        ...
        if swap_succeeds:
            try:
                ...
                logging.warning(
                    f"BSM operation failed at {self.node.name}: {e}")
            ...
            try:
                d1 = int(f1.get('depth', 0))
            except Exception:
                d1 = 0
            try:
                d2 = int(f2.get('depth', 0))
            except Exception:
                d2 = 0
            ...
            msg_left = ('SWAP', token_id, self.node_id, node2, src1, src2,
                        agg_z, agg_x, agg_depth)
            msg_right = ('SWAP', token_id, self.node_id, node1, src2, src1,
                         agg_z, agg_x, agg_depth)
            ...
            if self.successful_swaps % self.swap_log_every == 0:
                logging.info(
                    f"SWAP: {self.node.name} depth {agg_depth} Z:{agg_z} X:{agg_x} between Node_{node1} and Node_{node2}; token {token_id}"
                )
        else:
            try:
                qapi.measure(qubit1)
    ...
    def _handle_classical_with_node(self, message, from_id):
        try:
            ...
            if tag == 'PAIR':
                ...
                elif self.left_neighbor is not None and self.right_neighbor is not None:
                    other = self.right_neighbor if from_id == self.left_neighbor else self.left_neighbor
                    out_port = f"cout_{other}"
                else:
                    candidates = [
                        n for n in self.connected_nodes if n != from_id
                    ]
                ...
                if out_port and out_port in self.node.ports:
                    ...
                    logging.warning(
                        f"{self.node.name} cannot forward PAIR token {token_id}"
                    )
                return
            ...
            if tag == 'SWAP':
                ...
                _, token_id, swapper_id, partner_id, my_source, partner_source, parity_z, parity_x, depth = payload
                try:
                    parity_z = int(parity_z)
                except Exception:
                    parity_z = 0
                try:
                    parity_x = int(parity_x)
                except Exception:
                    parity_x = 0
                try:
                    depth = int(depth)
                ...
                if self.left_neighbor is not None and self.right_neighbor is not None:
                    ...
                    if out_port in self.node.ports:
                        self.node.ports[out_port].tx_output(
                            ('SWAP', token_id, swapper_id, partner_id,
                             my_source, partner_source, parity_z, parity_x,
                             depth))
                return
        except Exception as e:
            logging.warning(
                f"Error handling classical message at {self.node.name}: {e}")
...
def create_quantum_channel(length_km, loss_per_km, detector_efficiency=1.0):
    ...
    channel = QuantumChannel("QuantumChannel",
                             length=length_km,
                             models={
                                 "quantum_loss_model":
                                     FibreLossModel(p_loss_init=0,
                                                    p_loss_length=loss_per_km),
                                 "quantum_noise_model":
                                     DepolarNoiseModel(depolar_rate=1 -
                                                       detector_efficiency)
                             })
    return channel
...
def calculate_qber(original_bits, key_bits):
    ...
    # Count errors
    errors = sum(
        1 for idx in common_indices if original_bits[idx] != key_bits[idx])
    return errors / len(common_indices)


def create_qkd_network(num_nodes, channel_length_km, detector_efficiency,
                       loss_per_km, topology):
    ...
    for (node1_id, node2_id), source_node in source_lookup.items():
        ...
        q_chan_1 = create_quantum_channel(channel_length_km / 2, loss_per_km,
                                          detector_efficiency)
        q_conn_1 = DirectConnection(f"q_conn_{source_node.name}_{node1_id}",
                                    channel_AtoB=q_chan_1)

        q_chan_2 = create_quantum_channel(channel_length_km / 2, loss_per_km,
                                          detector_efficiency)
        q_conn_2 = DirectConnection(f"q_conn_{source_node.name}_{node2_id}",
                                    channel_AtoB=q_chan_2)

        # Add quantum connections from source to each node
        network.add_connection(source_node,
                               nodes[node1_id],
                               connection=q_conn_1,
                               label="quantum",
        ...
        c_conn_12 = DirectConnection(f"c_conn_{node1_id}_{node2_id}",
                                     channel_AtoB=c_chan_12)

        c_chan_21 = create_classical_channel(channel_length_km)
        c_conn_21 = DirectConnection(f"c_conn_{node2_id}_{node1_id}",
                                     channel_AtoB=c_chan_21)

        # Add classical connections between nodes
        network.add_connection(nodes[node1_id],
                               nodes[node2_id],
        ...
        network.add_connection(nodes[node2_id],
                               nodes[node1_id],
...
def visualize_network(network, topology="fully_connected"):
    ...
    if topology == "ring":
            ...
            try:
                ...
                if len(parts) >= 3:
                    ...
                    if node1 in pos_regular and node2 in pos_regular:
                        ...
                        pos[source] = (
                            (pos_regular[node1][0] + pos_regular[node2][0]) / 2,
                            (pos_regular[node1][1] + pos_regular[node2][1]) / 2)
                    else:

                        pos[source] = (0.1 * (random.random() - 0.5),
        ...
        pos_regular = {
            regular_nodes[i]: (x_positions[i], y_position)
        ...
        pos_regular = {
            central_node: (0, 0)
        } if central_node in regular_nodes else {}
    ...
    for u, v, key, data in G.edges(keys=True, data=True):
        ...
        curve = "arc3,rad=0.2" if data[
            'label'] == "Quantum Channel" else "arc3,rad=-0.2"
    ...
    output_path = os.path.join(OUTPUT_DIR,
                               f"eb_distributed_source_network_{topology}.png")
...
def visualize_p2p_network(network):
    ...
    pos = {"Node_0": (-1, 0), "Source_0_1": (0, 0), "Node_1": (1, 0)}
    ...
    plt.title("P2P EB-QKD Network with Entanglement Source in Middle",
              fontsize=22,
              fontweight='bold')
    plt.axis('off')
    plt.tight_layout()
    ...
    # Save figure
    output_path = os.path.join(OUTPUT_DIR, "p2p_eb_qkd_network.png")
    plt.savefig(output_path, bbox_inches='tight', dpi=300)
...
def visualize_star_central_source_quantum_only(num_leaves=5):
    ...
    for i, leaf in enumerate(leaves, start=1):
        leaf.add_ports(["qin_Source_Center"])

        # Create quantum channel only
        q_chan = create_quantum_channel(length_km=1,
                                        loss_per_km=0.2,
                                        detector_efficiency=0.9)
        q_conn = DirectConnection(f"q_conn_Source_Center_{i}",
                                  channel_AtoB=q_chan)
    ...
    # Slightly larger radius to reduce squish
    radius = 1.3
    # Use deterministic order of leaves to place them uniformly
    for idx, leaf in enumerate(leaves):
        angle = idx * angle_step
        pos[leaf.name] = (radius * np.cos(angle), radius * np.sin(angle))

    # Rotate the whole network slightly to create a gap for the legend
    theta = np.deg2rad(18)  # ~18 degrees rotation
    cos_t, sin_t = np.cos(theta), np.sin(theta)
    def _rot(p):
        return (p[0] * cos_t - p[1] * sin_t, p[0] * sin_t + p[1] * cos_t)
    pos = {k: _rot(v) for k, v in pos.items()}

    # Tiny left shift to increase clearance from the legend
    pos = {k: (v[0] - 0.05, v[1]) for k, v in pos.items()}
    ...
    # Place legend inside the axes in the new gap
    plt.legend(handles=legend_elements,
               loc='upper right',
               bbox_to_anchor=(0.98, 0.98),
               fontsize=16,
    ...
    plt.title("EB-QKD Star Network (Central Source)",
              ...
              fontweight='bold')
    plt.axis('off')
    # Use default tight_layout to avoid squeezing the axes
    plt.tight_layout()
...
def run_p2p_simulation(channel_length_km, detector_efficiency, loss_per_km,
                       simulation_duration):
    ...
    # Create quantum channels from source to each node (half the total distance each)
    q_chan_0 = create_quantum_channel(channel_length_km / 2, loss_per_km,
                                      detector_efficiency)
    q_conn_0 = DirectConnection("q_conn_Source_0_1_0", channel_AtoB=q_chan_0)

    q_chan_1 = create_quantum_channel(channel_length_km / 2, loss_per_km,
                                      detector_efficiency)
    q_conn_1 = DirectConnection("q_conn_Source_0_1_1", channel_AtoB=q_chan_1)
    ...
    c_conn_01 = DirectConnection("c_conn_0_1", channel_AtoB=c_chan_01)

    c_chan_10 = create_classical_channel(channel_length_km)
    c_conn_10 = DirectConnection("c_conn_1_0", channel_AtoB=c_chan_10)

    # Add classical connections
    network.add_connection(node0,
                           node1,
    ...
    # Source protocol
    source_protocol = EntanglementSourceProtocol(source, [(0, 1)],
                                                 pair_generation_rate=1.0)
    ...
    if key_id in shared_keys:
        ...
        if hasattr(node0_protocol, 'original_bits') and hasattr(
                node0_protocol, 'key_bits'):
            if 1 in node0_protocol.original_bits and 1 in node0_protocol.key_bits:
                qber = calculate_qber(node0_protocol.original_bits[1],
                                      node0_protocol.key_bits[1])
    else:
        key_length = 0
        key_rate = 0
        qber = 0
    ...
    return {
        ...
        "photon_travel": channel_length_km /
                         2  # Each photon travels half the distance
    }
...
def run_p2p_distance_sweep():
    ...
    for distance in distances:
        ...
        result = run_p2p_simulation(distance, detector_efficiency, loss_per_km,
                                    simulation_duration)
...
def run_simulation(num_nodes, channel_length_km, detector_efficiency,
                   loss_per_km, topology, simulation_duration):
    """Run an EB-QKD simulation with the specified parameters."""
    logging.info(
        f"Starting simulation: {topology} topology, {channel_length_km}km")

    # Reset NetSquid
    ns.sim_reset()

    network, nodes, entanglement_connections = create_qkd_network(
        num_nodes, channel_length_km, detector_efficiency, loss_per_km,
        topology)
    ...
    for source_node, connections in entanglement_connections.items():
        source_protocol = EntanglementSourceProtocol(source_node,
                                                     connections,
                                                     pair_generation_rate=1.0)
        protocols.append(source_protocol)
    ...
    for i in range(num_nodes):
        if connected_nodes_map[i]:
            node_protocol = NodeMeasurementProtocol(nodes[i], i, shared_keys,
                                                    connected_nodes_map[i])
    ...
    for key_id, key in shared_keys.items():
        ...
        if node1 in node_measurements and node2 in node_measurements:
            ...
            if node2 in node_measurements[
                    node1].original_bits and node2 in node_measurements[
                        node1].key_bits:
            ...
            elif node1 in node_measurements[
                    node2].original_bits and node1 in node_measurements[
                        node2].key_bits:
        ...
        logging.info(
            f"Key {key_id}: {len(key)} bits, Rate: {rate:.4f} bits/time unit, QBER: {qbers[key_id]:.4f}"
        )
    ...
    results = {
        ...
        "photon_travel_km": channel_length_km /
                            2,  # Each photon travels half the distance
...
def run_distance_sweep(num_nodes, distances, detector_efficiency, loss_per_km,
                       topology, simulation_duration):
    ...
    for idx, distance in enumerate(distances, start=1):
        ...
        sim_results = run_simulation(num_nodes, distance, detector_efficiency,
                                     loss_per_km, topology, simulation_duration)
...
def create_topology_comparison_plots(all_results, distances, topologies):
    ...
    for topology in topologies:
        ...
        for distance in distances:
            if distance in all_results[topology]:
                ...
                actual_distances.append(
                    all_results[topology][distance]["photon_travel_km"])
            else:
                rates.append(0)
        ...
        plt.plot(
            ...
            label=
            f'{topology.capitalize()} (Photon travel: {actual_distances[0]}km)')

    plt.xlabel('End-to-End Distance (km)', fontsize=14)
    plt.ylabel('Average Key Rate (bits/unit time)', fontsize=14)
    plt.title(
        'Key Rate Comparison - Distributed Source Model\nPhotons travel only half the total distance',
        fontsize=16,
        fontweight='bold')
    plt.grid(True)
    plt.legend()
...
def run_multi_hop_simulation(topology,
                             num_nodes,
                             total_distance,
                             num_hops,
                             detector_efficiency=0.9,
                             ...
                             target_bits=256):
    logging.info(
        f"Starting multi-hop simulation: {topology} topology, {total_distance}km, {num_hops} hops"
    )
    ns.sim_reset()
    if seed is not None:
        try:
            ns.set_random_state(int(seed))
        except Exception:
            pass
        try:
    ...
    if num_nodes < total_required_nodes:
        ...
        logging.info(
            f"Increasing number of nodes to {num_nodes} to accommodate {num_hops} hops"
        )
    ...
    logging.info(
        f"Selected path: {path}, end nodes: {end_nodes}, swap nodes: {swap_nodes}"
    )
    ...
    for idx, node_id in enumerate(path):
        if idx > 0:
            node_connections[node_id].append(path[idx - 1])
        if idx < len(path) - 1:
            node_connections[node_id].append(path[idx + 1])
    ...
    for a, b in path_edges:
        ...
        if source_node is None:
            ...
            continue
        protocols.append(
            EntanglementSourceProtocol(
                source_node, [(a, b)],
                pair_generation_rate=pair_generation_rate,
                downstream_ready_ports=None))
        ...
        for node_id in swap_nodes:
            protocols.append(
                EntanglementSwappingProtocol(
                    ...
                    path=path,
                    num_hops=num_hops,
                    upstream_ready_ports={}))

    # Ensure end-to-end classical connectivity between endpoints
    try:
        # add missing ports
        for a, b in [(end_a, end_b), (end_b, end_a)]:
            if f"cout_{b}" not in nodes[a].ports:
                nodes[a].add_ports([f"cout_{b}"])
            if f"cin_{b}" not in nodes[a].ports:
                nodes[a].add_ports([f"cin_{b}"])
        # channels
        c_ab = create_classical_channel(total_distance)
        c_ba = create_classical_channel(total_distance)
        conn_ab = DirectConnection(f"c_conn_{end_a}_{end_b}_e2e",
                                   channel_AtoB=c_ab)
        conn_ba = DirectConnection(f"c_conn_{end_b}_{end_a}_e2e",
                                   channel_AtoB=c_ba)
        network.add_connection(nodes[end_a],
                               nodes[end_b],
                               connection=conn_ab,
                               label="classical",
                               port_name_node1=f"cout_{end_b}",
                               port_name_node2=f"cin_{end_a}")
        network.add_connection(nodes[end_b],
                               nodes[end_a],
                               connection=conn_ba,
                               label="classical",
                               port_name_node1=f"cout_{end_a}",
                               port_name_node2=f"cin_{end_b}")
    except Exception as e:
    ...
    for i in range(num_chunks):
        try:
            ...
            break
        if target_bits and e2e_key_id in shared_keys and len(
                shared_keys[e2e_key_id]) >= target_bits:
...
def run_multi_hop_distance_sweep(topology,
                                 num_nodes,
                                 distances,
                                 num_hops_list,
                                 detector_efficiency=0.9,
    ...
    for distance in distances:
        for hops in num_hops_list:
            ...
            for t in range(num_trials):
                seed = (hash(topology) & 0xFFFF) * 10**6 + hops * 10**4 + int(
                    distance * 10) + t
            ...
            rate_std = float(
                np.std(trial_rates)) if len(trial_rates) > 1 else 0.0
...
def plot_multi_hop_comparison(results_by_topology,
                              distances,
                              num_hops_list,
...
def plot_key_rate_vs_hops(results_by_topology,
                          distances,
                          num_hops_list,
                          output_dir=OUTPUT_DIR):
    plt.figure(figsize=(8, 6))
    ...
    for topology, res in results_by_topology.items():
        ...
        for h in num_hops_list:
            if h in res:
                rates = [
                    res[h][d].get("end_to_end_key_rate", 0.0)
                    for d in distances
                    if d in res[h]
                ]
    ...
    plt.ylabel('Average End-to-End Key Exchange Rate (bits/time unit)',
               fontsize=11)
    plt.title('Key Exchange Rate vs Number of Swapping Nodes',
              fontsize=16,
              fontweight='bold')
    plt.legend(fontsize=10, loc='upper right')
    plt.grid(True)
...
def run_multi_hop_analysis():
    ...
    distances = [10, 20, 30, 40] + list(range(50, 1001,
                                              50))  # include short distances
    ...
    plot_multi_hop_comparison(all_results,
                              distances,
                              flat_hops,
                              output_dir=OUTPUT_DIR)
    plot_key_rate_vs_hops(all_results,
                          distances,
                          flat_hops,
                          output_dir=OUTPUT_DIR)
    ...
    for topology, res in all_results.items():
        ...
        for h in hops_by_topology[topology]:
            rates = [
                res[h][d]["end_to_end_key_rate"]
                for d in distances
                if d in res[h]
            ]
            if rates:
                logging.info(
                    f"  {h} hop{'s' if h != 1 else ''}: avg rate = {sum(rates)/len(rates):.6f} bits/unit"
                )
...
def main():
    ...
    parser.add_argument("--multihop",
                        dest="multihop",
                        action="store_true",
                        help="Run multi-hop analysis")
    parser.add_argument("--no-multihop",
                        dest="multihop",
                        action="store_false",
    ...
    for topology in topologies:
        ...
        network, nodes, _ = create_qkd_network(num_nodes, 20,
                                               detector_efficiency, loss_per_km,
                                               topology)
        visualize_network(network, topology=topology)

    visualize_star_central_source_quantum_only(num_leaves=6)
    ...
    for topology in topologies:
        ...
        results = run_distance_sweep(num_nodes, distances, detector_efficiency,
                                     loss_per_km, topology, simulation_duration)
    ...
    for topology in topologies:
        ...
        for distance, result in all_results[topology].items():
            logging.info(
                f"  Distance: {distance}km, Rate: {result['avg_rate']:.4f} bits/time unit, Total bits: {result['total_bits']}, QBER: {result['avg_qber']:.4f}"
            )
    ...
    if run_multihop:
        ...
        logging.info(
            "Skipping multi-hop analysis (pass --multihop or set EB_RUN_MULTI_HOP=1 to enable)."
        )
