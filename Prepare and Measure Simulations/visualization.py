...
def visualize_network(network, topology="fully_connected", output_dir=None):
    ...
    for u, v, key, data in G.edges(keys=True, data=True):
        ...
        curve = "arc3,rad=0.3" if data[
            'label'] == "Quantum Channel" else "arc3,rad=-0.3"
    ...
    output_path = os.path.join(output_dir,
                               f"network_visualisation_{topology}.png")
...
def create_total_distance_plots(all_results,
                                topologies,
                                output_dir,
                                node_pairs,
                                num_nodes,
                                max_distance=150,
                                topology_colors=None):
        ...
        for channel_length in sorted(all_results.keys()):
            if topology in all_results[channel_length]:
                for i, rate in enumerate(
                        all_results[channel_length][topology]["rates"]):
                    try:
                        total_dist = all_results[channel_length][topology][
                            "total_distances"][i]
                        ...
                        if i < len(node_pairs):
                            node_pair = node_pairs[topology][i] if isinstance(
                                node_pairs, dict) else node_pairs[i]
                            from network import calculate_path
                            path = calculate_path(node_pair[0], node_pair[1],
                                                  num_nodes, topology)
                            hop_count = len(path) - 1
                        else:
                            hop_count = 1  # Default if node_pair info is not available

                        topology_data[topology]['distances'].append(
                            channel_length)
                        ...
                        topology_data[topology]['total_distances'].append(
                            total_dist)
    ...
    baseline_distances = [
        d for d in sorted(all_results.keys()) if d <= max_distance
    ]
    baseline_rates = [
        all_results[d]["baseline"]["rates"][0] for d in baseline_distances
    ]
    plt.plot(baseline_distances,
             baseline_rates,
    ...
    for topology in topologies:
        ...
        # Group by hop count
        by_hops = {}
        for dist, rate, hops, total_dist in zip(data['distances'],
                                                data['rates'],
                                                data['hop_counts'],
        ...
        for hops, hop_data in sorted(by_hops.items()):
            ...
            sorted_distances = [
                hop_data['total_distances'][i] for i in sorted_indices
            ]
    ...
    sorted_pairs = sorted(zip(handles, labels, strict=False), key=sort_key)
    sorted_handles, sorted_labels = zip(*sorted_pairs, strict=False)
    ...
    # Plot baseline QBER
    baseline_qbers = [
        all_results[d]["baseline"]["qbers"][0] for d in baseline_distances
    ]
    plt.plot(baseline_distances,
             baseline_qbers,
             marker=hop_markers['baseline'],
    ...
    for topology in topologies:
        ...
        # Group by hop count
        by_hops = {}
        for dist, qber, hops, total_dist in zip(data['distances'],
                                                data['qbers'],
                                                data['hop_counts'],
                                                data['total_distances'],
                                                strict=False):
