...
import numpy as np
from network import calculate_path, create_qkd_network
...
def calculate_qber(sent_bits,
    ...
    if sent_bases is not None and recv_bases is not None and sent_ids is not None and recv_ids is not None:
        tx = {
            pid: (bit, bas) for pid, bit, bas in zip(
                sent_ids, sent_bits, sent_bases, strict=False)
        }
        rx = {
            pid: (meas, bas) for pid, meas, bas in zip(
                recv_ids, measured_bits, recv_bases, strict=False)
        }
    ...
    errors = sum(1 for s, m in zip(
        sent_bits[:min_length], measured_bits[:min_length], strict=False)
                 if s != m)
...
def run_simulation(num_nodes,
    ...
    network, nodes = create_qkd_network(num_nodes, channel_length_km,
    ...
    for (node1, node2) in node_pairs:
        ...
        logging.info(
            f"Path Node_{node1}->{node2}, hops={len(path)-1}, total_distance={total_distance}km"
        )

        if len(path) == 2:
            protocols = [
                TransmitterProtocol(nodes[node1],
                                    final_key,
                                    sent_bits,
                                    node2,
            ...
            while elapsed_time < max_duration and len(
                    final_key) < target_key_length:
            ...
            key_exchange_rate = (secure_len / units_to_seconds(
                elapsed_time if elapsed_time > 0 else simulation_duration))
        else:
            logging.info(
                f"Trusted-node multi-hop over {len(path)-1} hops with target={target_key_length}"
            )
            num_hops = len(path) - 1
            ...
            for i in range(num_hops):
                src = path[i]
                dst = path[i + 1]
                tx = TransmitterProtocol(nodes[src],
                                         hop_keys[i],
                                         hop_sent_bits[i],
                                         dst,
                                         target_length=target_key_length,
                                         sent_bases=hop_sent_bases[i],
                                         pulse_ids=hop_tx_ids[i])
                rx = ReceiverProtocol(nodes[dst],
                                      hop_keys[i],
                                      hop_sent_bits[i],
                                      hop_measured_bits[i],
                                      src,
                                      target_length=target_key_length,
                                      detection_efficiency=detector_efficiency,
                                      recv_bases=hop_recv_bases[i],
                                      recv_pulse_ids=hop_rx_ids[i])
            ...
            while elapsed_time < max_duration:
                ...
                elapsed_time += time_increment
                for i in range(num_hops):
                    if hop_done_time[i] is None and len(
                            hop_keys[i]) >= target_key_length:
            ...
            for i in range(num_hops):
                ...
                hop_secure_lengths.append(
                    estimate_secure_key_len(target_key_length, q_i))
            min_secure_len = int(
                min(hop_secure_lengths)) if hop_secure_lengths else 0
            slowest_time = max(t for t in hop_done_time
                               if t is not None) if any(hop_done_time) else 0
            key_exchange_rate = (
                min_secure_len /
                units_to_seconds(slowest_time)) if slowest_time > 0 else 0.0
            qber = max(hop_qbers) if hop_qbers else 0.0
            final_key = hop_keys[
                0][:min_secure_len] if min_secure_len > 0 else []
            hop_statistics.append({
                'path': path.copy(),
                'distances': [channel_length_km] * num_hops,
                'hop_rates': [
                    (estimate_secure_key_len(target_key_length, hop_qbers[i]) /
                     units_to_seconds(hop_done_time[i]))
                    if hop_done_time[i] else 0.0 for i in range(num_hops)
                ],
        ...
        logging.info(
            f"End-to-end secure key rate: {key_exchange_rate:.6f} bits/s, QBER: {qber:.4f}, key_len={len(final_key)}"
        )
