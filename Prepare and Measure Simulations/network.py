...
def create_quantum_channel(length_km, loss_per_km):
    return QuantumChannel("QuantumChannel",
                          length=length_km,
                          models={
                              "quantum_loss_model":
...
def create_network_connections(network, nodes, channel_length_km, loss_per_km,
                               topology):
    if topology == "fully_connected":
        for i in range(len(nodes)):
            for j in range(i + 1, len(nodes)):
                q_channel_ij = create_quantum_channel(channel_length_km,
                                                      loss_per_km)
                q_conn_ij = DirectConnection(f"quantum_connection_{i}_{j}",
                                             channel_AtoB=q_channel_ij)
                ...
                q_channel_ji = create_quantum_channel(channel_length_km,
                                                      loss_per_km)
                q_conn_ji = DirectConnection(f"quantum_connection_{j}_{i}",
                                             channel_AtoB=q_channel_ji)
                network.add_connection(nodes[j],
                                       nodes[i],
                ...
                c_conn_ij = DirectConnection(f"classical_connection_{i}_{j}",
                                             channel_AtoB=c_channel_ij)
                network.add_connection(nodes[i],
                                       nodes[j],
                ...
                c_conn_ji = DirectConnection(f"classical_connection_{j}_{i}",
                                             channel_AtoB=c_channel_ji)
                network.add_connection(nodes[j],
                                       nodes[i],
        ...
        for i in range(1, len(nodes)):
            q_channel_0i = create_quantum_channel(channel_length_km,
                                                  loss_per_km)
            q_conn_0i = DirectConnection(f"quantum_connection_0_{i}",
                                         channel_AtoB=q_channel_0i)
            network.add_connection(central_node,
                                   nodes[i],
                                   connection=q_conn_0i,
                                   label="quantum",
                                   port_name_node1=f"qout_{i}",
                                   port_name_node2="qin_0")
            q_channel_i0 = create_quantum_channel(channel_length_km,
                                                  loss_per_km)
            q_conn_i0 = DirectConnection(f"quantum_connection_{i}_0",
                                         channel_AtoB=q_channel_i0)
            network.add_connection(nodes[i],
                                   central_node,
            ...
            c_conn_0i = DirectConnection(f"classical_connection_0_{i}",
                                         channel_AtoB=c_channel_0i)
            network.add_connection(central_node,
                                   nodes[i],
            ...
            c_conn_i0 = DirectConnection(f"classical_connection_{i}_0",
                                         channel_AtoB=c_channel_i0)
            network.add_connection(nodes[i],
                                   central_node,
...
def create_qkd_network(num_nodes, channel_length_km, detector_efficiency,
    ...
    create_network_connections(network, nodes, channel_length_km, loss_per_km,
                               topology)
...
def calculate_ring_path(source, target, num_nodes):
    ...
    if clockwise_distance <= counter_clockwise_distance:
        ...
        path = [(source - i) % num_nodes
                for i in range(counter_clockwise_distance + 1)]
    return path
