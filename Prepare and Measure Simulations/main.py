...
from simulation import TIME_UNIT_SECONDS, calculate_qber, run_simulation
from visualization import create_total_distance_plots, visualize_network
...
def main():
    ...
    channel_distances = (list(range(5, 101, 5)) + list(range(110, 151, 10)) +
    ...
    for channel_length_km in channel_distances:
        ...
        baseline_network, baseline_nodes = create_qkd_network(
            2, channel_length_km, detector_efficiency, loss_per_km, "bus")
        ...
        key_exchange_rate = len(final_key) / (simulation_duration *
                                              TIME_UNIT_SECONDS)
        qber = calculate_qber(sent_bits, measured_bits, sent_bases, recv_bases,
                              tx_ids, rx_ids)
        ...
        logging.info(
            f"Baseline P2P simulation completed. Total distance: {channel_length_km}km, Key rate: {key_exchange_rate:.4f}, QBER: {qber:.4f}"
        )

    create_baseline_plots(sorted(list(baseline_rates.keys())), baseline_rates,
                          baseline_qbers, OUTPUT_DIR, topology_colors,
                          hop_markers)
    ...
    for topology in topologies:
        ...
        vis_network, _ = create_qkd_network(num_nodes, 20, detector_efficiency,
    ...
    for channel_length_km in channel_distances:
        ...
        for topology in topologies:
            ...
            for i, (node_pair, key, qber, total_dist) in enumerate(
                    zip(node_pairs, keys, qbers, total_distances,
                        strict=False)):
                distance = distances[i]
                key_preview = key[:10] if key else []
                logging.info(
                    f"[{channel_length_km}km per hop, {total_dist}km total] Final Key for nodes {node_pair} ({distance}): {key_preview}... (length: {len(key)}, QBER: {qber:.4f})"
                )

        create_distance_comparison_plots(channel_length_km, topology_rates,
                                         topology_qbers, topology_distances,
                                         topology_total_distances, OUTPUT_DIR,
                                         topology_colors)

    create_adjacent_nodes_comparison(all_results, topologies, channel_distances,
                                     OUTPUT_DIR, topology_colors, hop_markers)

    for topology in topologies:
        create_topology_vs_baseline_plots(
            all_results,
            topology, [d for d in channel_distances if d <= 150],
            OUTPUT_DIR,
            topology_colors,
            hop_markers,
            max_distance=150)
        create_topology_vs_baseline_plots(
            all_results,
            topology, [d for d in channel_distances if d <= 250],
            OUTPUT_DIR,
            topology_colors,
            hop_markers,
            max_distance=250)
        for max_d in [350, 450, 550, 650]:
            create_topology_vs_baseline_plots(
                all_results,
                topology, [d for d in channel_distances if d <= max_d],
                OUTPUT_DIR,
                topology_colors,
                hop_markers,
                max_distance=max_d)

    create_total_distance_plots(all_results,
                                topologies,
                                OUTPUT_DIR,
                                topology_node_pairs,
                                num_nodes,
